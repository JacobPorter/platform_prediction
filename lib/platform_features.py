...
def quality_features(vector, reduced=False):
    ...
    my_max = max(vector)
    my_min = min(vector)
    variance = float(sum([math.pow(item - mean, 2) for item in vector])) / len(vector)
    # Division by zero error
    if reduced:
        return [mean, my_max, my_min, variance]
    try:
        skewness = float(
            sum([math.pow((item - mean) / (variance + 0.0), 3) for item in vector])
        ) / len(vector)
    except ZeroDivisionError:
        skewness = 0
    try:
        kurt = float(
            sum([math.pow((item - mean) / (variance + 0.0), 4) for item in vector])
        ) / len(vector)
    except ZeroDivisionError:
        kurt = 9 / 5.0
    diff = []
...
def get_features(
    ...
    for record in reader:
        ...
        if debug:
            print(
                "{} Qual len:{} Offset: {}".format(count, len(qual), offset),
                file=sys.stderr,
            )
            ...
            if debug:
                sys.stderr.write(
                    "{} Read len:{} Offset: {} Begin: {}, End: {} {} \n".format(
                        ...
                        i * halflength,
                        finallength,
                        qual[i * halflength : finallength],
                    )
                )
                sys.stderr.flush()
            features += get_qual_features(qual[i * halflength : finallength], reduced)
        if label and output:
